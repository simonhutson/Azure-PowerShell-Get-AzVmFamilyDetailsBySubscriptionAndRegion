...
$filteredVmSkus = foreach ($sku in $vmSkus) {
    ...
    [pscustomobject]@{
        VMFamily                       = $vmFamily
        CPUArchitecture                = Get-CpuArchitectureOutput -CapabilityMap $capabilityMap -VMFamily $vmFamily -SkuName $skuName
        SkuName                        = $skuName
        ...
        RetirementStatus               = Get-RetirementStatus -VMFamily $vmFamily -SkuName $skuName
...
if (-not $result) {
    Write-Warning "No matching Gen1- or Gen2-capable VM families were found in location '$Location' for DiskInterface='$DiskInterface' and LocalTemporaryDisk='$LocalTemporaryDisk'."
    return
}
