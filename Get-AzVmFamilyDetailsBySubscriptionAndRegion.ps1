...
.NOTES
    ...
    2) Local temporary/resource disk filter:
       - The script derives "has local temp/resource disk" from MaxResourceVolumeMB:
           > 0  => local temp/resource disk is present
           <= 0 or missing => local temp/resource disk is absent
...
.PARAMETER LocalTemporaryDisk
    ...
    Required = include only families whose SKUs have MaxResourceVolumeMB > 0
    Excluded = include only families whose SKUs have MaxResourceVolumeMB <= 0 or the capability is missing
...
function Get-HasLocalTemporaryDisk {
    ...
    # Derived script rule:
    # Use MaxResourceVolumeMB as the presence signal for a local temp/resource disk.
    ...
    return ($maxResourceVolumeMb -gt 0)
}
...
$filteredVmSkus = foreach ($sku in $vmSkus) {
    ...
    $hasTempDisk = Get-HasLocalTemporaryDisk -CapabilityMap $capabilityMap

    switch ($LocalTemporaryDisk) {
        'Required' {
            if (-not $hasTempDisk) { continue }
        }
        'Excluded' {
            if ($hasTempDisk) { continue }
        }
        'Any' {
            # no-op
        }
...
$result = $filteredVmSkus |
    ...
    ForEach-Object {
        ...
        [pscustomobject]@{
            ...
            HasLocalTemporaryDisk          = $representativeSku.HasLocalTemporaryDisk
